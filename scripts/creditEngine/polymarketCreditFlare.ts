...
export function computeCreditFromFlare(score: FlareScoreInput): CreditResult {
  ...
  const ROI_real =
    totalBoughtAll <= 0 ? 0 : totalRealizedPnl / Math.max(1, totalBoughtAll);

  const WinRate =
    N_closed === 0
      ? 0.5
      : closed.filter((cp) => cp.realizedPnl > 0).length /
        Math.max(1, N_closed);
  ...
  if (N_closed > 0) {
    ...
    if (N_closed > 1) {
      rVar =
        rj.reduce((sum, r) => sum + (r - rMean) * (r - rMean), 0) / N_closed;
    }
  }
  ...
  const DD_open =
    I_open <= 0 ? 0 : (C_open - I_open) / Math.max(1, I_open);
  ...
  const DeadShare =
    I_open <= 0 ? 0 : deadInitSum / Math.max(1, I_open);

  const V_open = C_open;
  const V_realized_plus = closed.reduce(
    (sum, cp) => sum + Math.max(0, cp.realizedPnl),
    0
  );
  ...
  const PerfScore =
    ...
    0.8 * (WinRate - 0.5) +
    0.4 * Sharpe +
    0.15 * ProfitFactorFeature;

  // Risk score:
  // Risk = 1.5*(-DD_open) + 1.0*HHI_open + 1.0*DeadShare
  const RiskScore =
    1.5 * (-DD_open) + 1.0 * HHI_open + 1.0 * DeadShare;
  ...
  const ltv =
    LTV_min +
    (LTV_max - LTV_min) * Math.pow(G, gamma);
  ...
  const maxLoan = Math.max(
    0,
    Math.min(collateralLoan, cappedLoan)
  );
